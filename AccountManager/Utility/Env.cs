...
namespace AccountManager
{
    ...
    { 
		...
		public static MvcHtmlString GetMenuBarPage(Nullable<int> ParentId)
        {
            ...
            sb.Append(GetMenuBar(ParentId, (MenuPermission[])globle,RoleId));
            sb.Append("</ul>");
        ...
        private static MvcHtmlString GetMenuBar(Nullable<int> ParentId, MenuPermission[] q,Nullable<int> RoleId)
        {
            StringBuilder sb = new StringBuilder();
            if (q != null)
            {
                foreach (var item in q.Where(i => i.Menu_MenuId.ParentId == ParentId && i.RoleId==RoleId).OrderBy(i => i.SortOrder))
                {
                    var js = q;
                    ...
                    if (js.Count(j => j.Menu_MenuId.ParentId == item.Menu_MenuId.Id) > 0)
                    {
                        ...
                        sb.Append(GetMenuBar(item.Menu_MenuId.Id, q,RoleId));
                    }
                    else
                    {
                        if (item.Menu_MenuId.ParentId == null)
